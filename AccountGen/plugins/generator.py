...
async def gen(event):
    if not (await check_user(event.sender_id)):
        return await event.reply(f"{strt}\n\nOops! You have to join the given channel so as to use me!", buttons=[Button.url("Join My Channel", url=ltc)])
...
buttons=[
    ...
    [Button.url("The Dynamic Network", url="https://t.me/TheDynamicNetwork")]
])
